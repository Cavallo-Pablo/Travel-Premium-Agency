...
@Component({
  selector: 'app-register-user-buy',
  templateUrl: './register-user-buy.component.html',
  styleUrls: ['./register-user-buy.component.css']
})
export class RegisterUserBuyComponent {
  listaUsuarios: User[] = [];
  ...
  altaUsuario() {
    ...
    this.apiService.postUser(usuario).subscribe({
      next: (post) => {
        ...
        this.router.navigate(['/loginUserBuy'])
      },
      error: (error) => {
        console.log(error)
      }
